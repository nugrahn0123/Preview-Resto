...
export default function MenuPage() {
  ...
  return (
    <main style={{ backgroundColor: '#F5D5B0', minHeight: '100vh' }}>
      ...
      <section className="py-8 md:py-10 px-4 md:px-6 max-w-6xl mx-auto">
        {/* Search */}
        <div className="relative max-w-md mx-auto mb-6 md:mb-8">
          <span className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 text-base" style={{ color: '#D4396B' }}>🔍</span>
          <input
            type="text"
            placeholder="Cari menu..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-9 md:pl-10 pr-4 py-2.5 md:py-3 rounded-full outline-none text-sm shadow-md"
            style={{
        ...
        {/* Category filter */}
        <div className="flex flex-wrap gap-1.5 md:gap-2 justify-center mb-8 md:mb-10">
          {categories.map((cat) => (
            <button
              key={cat}
              onClick={() => setActive(cat)}
              className="px-4 md:px-5 py-1.5 md:py-2 rounded-full text-xs md:text-sm font-semibold transition-all duration-200 active:scale-95"
              style={
                active === cat
        ...
        {/* Results count */}
        <p className="text-xs md:text-sm mb-4 md:mb-6 text-center" style={{ color: '#7a5565' }}>
